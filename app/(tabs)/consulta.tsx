  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.section}>
        <Text style={styles.sectionLabel}>{'/* parámetros de búsqueda */'}</Text>
        ...
        <TouchableOpacity style={styles.button} onPress={consultarNotas} disabled={loading}>
      ...
      {result && (
        <View>
          ...
          <View style={styles.resultBox}>
            <Text style={styles.sectionLabel}>{'/* datos del estudiante */'}</Text>
            <Text style={styles.resultLine}><Text style={styles.key}>nombre:  </Text><Text style={styles.val}>"{result.estudiante.nombre}"</Text></Text>
            <Text style={styles.resultLine}><Text style={styles.key}>cedula:  </Text><Text style={styles.val}>"{result.estudiante.cedula}"</Text></Text>
            <Text style={styles.resultLine}><Text style={styles.key}>correo:  </Text><Text style={styles.val}>"{result.estudiante.correo}"</Text></Text>
            <Text style={styles.resultLine}><Text style={styles.key}>celular: </Text><Text style={styles.val}>"{result.estudiante.celular}"</Text></Text>
          </View>
            ...
            result.notas.map((item, i) => (
              <View key={i} style={styles.notaCard}>
                <View style={styles.notaHeader}>
                  ...
                  <Text style={[styles.notaDef, { color: getColor(item.definitiva) }]}>
                    {item.definitiva?.toFixed(2)}
                  </Text>
                </View>
                <View style={styles.notaRow}>
                  {[item.nota1, item.nota2, item.nota3, item.nota4].map((n, j) => (
...
const styles = StyleSheet.create({
  ...
  key: { color: '#4a7c4a' },
  val: { color: '#00ff41' },
  notaCard: { backgroundColor: '#0d160d', borderWidth: 1, borderColor: '#1a3a1a', borderRadius: 4, padding: 16, marginBottom: 12 },
  notaHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  notaMateria: { fontFamily: 'monospace', fontSize: 14, color: '#00ff41', fontWeight: 'bold' },
  notaDef: { fontFamily: 'monospace', fontSize: 22, fontWeight: 'bold',
    textShadowOffset: { width: 0, height: 0 }, textShadowRadius: 8 },
  notaRow: { flexDirection: 'row', justifyContent: 'space-between' },
  notaItem: { alignItems: 'center', flex: 1 },
  notaItemLabel: { fontFamily: 'monospace', fontSize: 11, color: '#4a7c4a', marginBottom: 2 },
  notaItemVal: { fontFamily: 'monospace', fontSize: 16, color: '#a0d0a0' },
  emptyBox: { padding: 20, borderWidth: 1, borderColor: '#1a3a1a', borderStyle: 'dashed', borderRadius: 4, alignItems: 'center' },
  emptyText: { fontFamily: 'monospace', fontSize: 13, color: '#2a5a2a' },
  errorBox: { marginTop: 8, marginBottom: 16, padding: 12, backgroundColor: '#1a0000', borderLeftWidth: 3, borderLeftColor: '#ff3333', borderRadius: 2 },
